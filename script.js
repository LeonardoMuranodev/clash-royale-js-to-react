...
function crear_carta(carta) {
    return `
        <div>
            <h3> ${carta.name} </h3>
            <p>Maximo Nivel:  ${carta.maxLevel}</p>
            <p>Coste de elixir: ${carta.elixirCost}</p>
            <p>Rareza: ${carta.rarity}</p>
            <img src="${carta.iconUrls.medium}" alt=""></img>
            <button type="button">Ver Evo</button>
            <button type="button">Ver Heroe</button>
        </div>
    `
}
...
console.log(cartas.slice(0,3))

await renderizar_cartas(5)
